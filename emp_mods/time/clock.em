// time.clock (minimal)
// Monotonic time for benchmarking.

extern "C" fn QueryPerformanceCounter(lpPerformanceCount: *i64) -> i32;
extern "C" fn QueryPerformanceFrequency(lpFrequency: *i64) -> i32;

export fn nowNanos() -> i64 {
  let counter: i64 = 0;
  let freq: i64 = 0;
  let billion: i64 = 1000000000;
  let ok1: i32;
  let ok2: i32;
  @emp off {
    ok1 = QueryPerformanceCounter(&mut counter);
    ok2 = QueryPerformanceFrequency(&mut freq);
  }
  if ok1 == 0 { return (i64)0; }
  if ok2 == 0 { return (i64)0; }
  if freq <= 0 { return (i64)0; }

  // Avoid overflow in (counter * 1e9)
  let secs: i64 = counter / freq;
  let rem: i64 = counter % freq;
  let ns: i64 = secs * billion + (rem * billion) / freq;
  return ns;
}
