...
export fn fileOpen(u8[] path) -> FileResult {
...
export fn fileCreate(u8[] path) -> FileResult {
    let wpath: u16[] = utf8ToWide(path);
    let h: u64;
    @emp off {
        let wp: *u16 = &wpath[0];
...
export fn fileOpenAppend(u8[] path) -> FileResult {
    let wpath: u16[] = utf8ToWide(path);
    let h: u64;
    // Read & Write access
...
// Reads into a caller-provided raw buffer.
export fn fileReadInto(File f, *u8 p, i32 len) -> FileResult {
...
// Back-compat: list-based read (consumes buf under ownership rules).
export fn fileRead(File f, u8[] buf) -> FileResult {
    let len: i32 = buf.len();
    if len <= 0 { let r: FileResult; r.is_ok = true; r.value = 0; return r; }
    let p: *u8 = &buf[0];
    return fileReadInto(f, p, len);
}

export fn fileWrite(File f, u8[] data) -> FileResult {
...
export fn fileClose(File f) {
    @emp off { CloseHandle(f.handle); }
}

export fn fileSeek(File f, i64 offset, i32 whence) -> i64 {
    let h: u64 = f.handle;
    let success: i32;
    let newPos: i64 = 0;
...
export fn fileSync(File f) -> bool {
    let s: i32;
...
export fn fileDelete(u8[] path) -> FileResult {
    let wpath: u16[] = utf8ToWide(path);
    let success: i32;
...
export fn fileRename(u8[] oldPath, u8[] newPath) -> FileResult {
...
export fn fileCopy(u8[] src, u8[] dst, bool overwrite) -> FileResult {
...
export fn dirCreate(u8[] path) -> FileResult {
    let wpath: u16[] = utf8ToWide(path);
    let success: i32;
...
export fn dirRemove(u8[] path) -> FileResult {
    let wpath: u16[] = utf8ToWide(path);
    let success: i32;
...
export fn readDir(u8[] path) -> DirEntry[] {
...
export fn removeDirAll(u8[] path) -> FileResult {
...
export fn metadata(u8[] path) -> Metadata {
    let wpath: u16[] = utf8ToWide(path);
    let buffer: u8[];
